...
export const MascoteBlock: React.FC<MascoteBlockProps> = ({ state, energyLevel = 75, t }) => {
  ...
  return (
    <div className="glass-card p-6 flex flex-col items-center justify-center space-y-4 border-zenith-border-primary bg-zenith-surface-1 relative overflow-hidden group">
      <div className="flex items-center space-x-2 absolute top-4 left-4">
        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
        <span className="text-[8px] font-bold uppercase tracking-widest text-zenith-text-tertiary">Neural Link Active</span>
      </div>
      ...
      <motion.div
        ...
        className="relative"
      >
        <div className="w-24 h-24 rounded-full bg-zenith-surface-2 flex items-center justify-center border border-zenith-border-secondary relative z-10">
          {getIcon()}
        </div>
        <motion.div
      ...
      <div className="text-center space-y-1">
        <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-zenith-text-tertiary">{t.mascote.title}</h4>
        <p className="text-xl font-display font-bold text-zenith-text-primary uppercase tracking-tight">{getLabel()}</p>
      </div>

      <div className="w-full h-1 bg-zenith-surface-2 rounded-full overflow-hidden">
        <motion.div 
          initial={{ width: 0 }}
